...
const Leavefacedet = ({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTimeout(() => setState((prev) => ({ ...prev, submissionStatus: null })), 3000);
            } else {
                setState((prev) => ({
                    ...prev,
                    submissionStatus: "error",
                    errors: { ...prev.errors, general: data.message || "Failed to submit leave request." },
                }));
                setTimeout(() => setState((prev) => ({ ...prev, submissionStatus: null, errors: { ...prev.errors, general: "" } })), 3000);
            }
        } catch (error) {
            setState((prev) => ({
                ...prev,
                submissionStatus: "error",
                errors: { ...prev.errors, general: "Network error. Please try again." },
            }));
            setTimeout(() => setState((prev) => ({ ...prev, submissionStatus: null, errors: { ...prev.errors, general: "" } })), 3000);
    ...
    return (
        <div className="min-h-screen bg-[#F3F2EF] flex items-center justify-center p-6">
            <div className="relative max-w-4xl w-full bg-white rounded-xl p-8 flex flex-col md:flex-row gap-10 border border-gray-200">
                <div className="relative flex-shrink-0 w-full md:w-72">
                    {!state.faceDetected ? (
                        <>
                            ...
                            <div className="relative w-full md:w-72 h-72 rounded-full overflow-hidden border-2 border-[#0A66C2]/20 shadow-md">
                                {cameraLoading && <p className="text-center">Loading Camera...</p>}
                        ...
                        <div className="relative w-full md:w-72 h-72 rounded-full overflow-hidden border-4 border-[#00A0DC] shadow-[0_0_15px_rgba(0,160,220,0.3)] animate-fade-in">
                            <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover" />
