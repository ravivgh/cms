...
const MemoizedCardD = memo(CardD);

const Dashboard = ({ isSidebarOpen }) => {
    const profile_name = localStorage.getItem("Staff_name");
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                const response = await axios.post(
                    "http://localhost:5472/services/getcount",
                                ...
                                <div className=" flex items-center justify-between">
                                    <div className="hidden md:block">
                                        <div className="border-[#cbcbcba2] border-t-2 border-l-2 border-r-2 rounded-t-md p-1 relative">
                                            <div className="relative w-48 h-48 group overflow-hidden rounded-md">
                                                ...
                                                <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-opacity flex items-center justify-center">
                                                    ...
                                                    >
                                                        Profile
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="10" viewBox="0 0 18 12" fill="none" transform="rotate(-45)"><path d="M1 6H17M17 6L12 1M17 6L12 11" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path></svg>
                                                    </Link>
                                                </div>
                                            </div>
                                        </div>
                                        <div>
                                            <h1 className="text-white text-md text-center bg-[#1c65b7] rounded-b-md border-[#cbcbcba2] border-b-2 border-l-2 border-r-2 py-1 flex items-center justify-center gap-2">
                                                {profile_name}
                                                <span><VscVerifiedFilled className="text-[#eceded]" /></span>
                                            </h1>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 my-6 w-full md:w-4/5">
                                        <MemoizedCardD
                                            ...
                                            value={loading ? <Skeleton width={80} /> : dashboardData.Studentcount}
                                            description="Number of Total Students"
                                        />
                                        <MemoizedCardD
                                            icon={<MdOutlineGroups style={{ fontSize: "25px" }} />}
                                            title="Total Faculty"
                                            value={loading ? <Skeleton width={80} /> : dashboardData.staffcount}
                                            description="Number of Total Faculty"
                                        />
                                        <MemoizedCardD
                                            icon={<TrendingUp />}
                                            title="Present"
                                            value={loading ? <Skeleton width={80} /> : dashboardData.present}
                                            description="Number of Present"
                                        />
                                        <MemoizedCardD
                                            icon={<TrendingDown />}
                                            title="Absent"
                                            value={loading ? <Skeleton width={80} /> : dashboardData.absent}
