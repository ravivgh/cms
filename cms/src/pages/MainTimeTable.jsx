...
const TimeTable = () => {
  ...
  const fetchSubjects = async () => {
    ...
    if (studentId) {
      try {
        const response = await axios.post("http://localhost:5472/services/getsubjectsbysid", { sid: parseInt(studentId) });
      ...
      <div className="flex">
        ...
        >
          <div className="bg-[#f7f7f7] p-5 rounded-lg">
            <h1 className="text-black py-10 text-2xl text-center">Time Table</h1>
            <hr
              ...
              <table className="min-w-full border-collapse border border-gray-300">
                <thead>
                  <tr className="bg-gray-200">
                    <th className="border border-gray-300 p-2 text-black font-bold text-center">Days</th>
                    <th className="border border-gray-300 p-2 text-black font-bold text-center">Period 1</th>
                    <th className="border border-gray-300 p-2 text-black font-bold text-center">Period 2</th>
                    <th className="border border-gray-300 p-2 text-black font-bold text-center">Period 3</th>
                    <th className="border border-gray-300 p-2 text-black font-bold text-center">Period 4</th>
                  </tr>
                </thead>
                <tbody>
                  {timetable.map((entry) => (
                    <tr key={entry.day} className="hover:bg-gray-100">
                      <td className="border border-gray-300 p-2 text-black text-center">{entry.day}</td>
                      {entry.periods.map((subject, index) => (
                        <td key={index} className="border border-gray-300 p-2 text-black text-center">{subject}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
