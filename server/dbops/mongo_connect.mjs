...
app.post('/services/updaterec',async (req,res) =>{
  ...
  try {
    ...
    if(documents.acknowledged && documents.modifiedCount > 0 && documents.matchedCount > 1){
...
const db = new mongocon();
  ...
  try {
    ...
    const collection = database.collection(collename); 
     genid()
    const result = await collection.insertOne({_id: uid,college_name : colname,address :addr});
...
app.post('/services/insertstaff',async (req,res) =>{
  const {staffname,Class,section,subject,email,phone,college} = req.body
...
const db = new mongocon();
  ...
  try {
    ...
    const collection = database.collection("Staff_Master"); 
     genid()
    const result = await collection.insertOne({_id: uid,Staff_name : staffname,Assigned_Class: Class,Section:section,Subject: subject,Staff_Email : email,Mob: phone,College_id:college});
    if(result.acknowledged == true && result.insertedId == uid)
    {
...
const db = new mongocon();
  ...
  try {
    ...
    const collection = database.collection("Student_Master"); 
     genid()
    const result = await collection.insertOne({_id: parseInt(uid),Student_Name : name,Class: Class,Section:section,DOB:d_o_b,Email:email,Mobile: phone,College_id:college});
    if(result.acknowledged == true && result.insertedId == uid)
    {
...
app.post('/services/getattendance', async (req, res) => {
  ...
  try {
    await db.client.connect(); 
    const database = db.client.db(db.dbname);
    const attendance = await database.collection('Attendance_Master').find({
      Subject: subject,
...
const db = new mongocon();
  ...
  try {
    ...
    const collection = database.collection(collecname); 
     genid()
    const result = await collection.insertOne({_id: uid,College_id:college_id,Staff_name : Sname,Pass_word :Password,E_mail:Email,Mob:Mobile});
    if(result.acknowledged == true && result.insertedId == uid)
    {
...
app.post('/services/addstaff', async (req, res) => {
  ...
  try {
    ...
    if (nonExistingStaff.length > 0) {
      const result = await collection.insertMany(nonExistingStaff, { ordered: false });

      if (result.acknowledged === true && result.insertedCount > 0) {
        res.status(200).json({ Message: "Successful", InsertedCount: result.insertedCount });
      } else {
        res.status(400).json({ Message: "Error" });
      }
    } else {
      res.status(400).json({ Message: "No new staff to add. All emails already exist." });
    }
  } catch (error) {
    console.error('Error inserting documents:', error);
...
app.post('/services/retrievestaffs', async (req, res) => {
  ...
  try {
    ...
    if (staff.length > 0) {
      ...
      res.status(300).json({ "Message": "No staff found" });
    }
  } catch (error) {
...
app.post('/services/retrievestudentsbyid', async (req, res) => {
  ...
  try {
    ...
    const staff = await collection.find({_id:searchwith}, {
      projection: {
        Student_Name: 1,
        Class: 1,
        Section: 1,
...
app.post('/services/retrievestaffbystudid', async (req, res) => {
  ...
  try {
      ...
      const studdetails = await studcollection.findOne({ _id: parseInt(searchwith) });

      if (studdetails) { 
          const staffdetails = await staffcollection.find({
...
app.post('/services/deletestaff', async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({ message: 'Record deleted successfully', deletedRecord });
  } catch (error) {
...
app.post('/services/getcount', async (req, res) => {
  ...
  try {
    ...
    if (!staffCount && !studentCount && !subject && !absentCount && !presentCount) {
...
app.post('/services/getstafdashcount', async (req, res) => {
  ...
  try {
    ...
    let presentPercentage = Math.ceil((attendanceCount / totalAttendanceOpportunities) * 100); 
    let absentPercentage = Math.ceil(((totalAttendanceOpportunities - attendanceCount) / totalAttendanceOpportunities) * 100); 

    if (!staffCount) {
      return res.status(200).send({ message: 'Staff record not found' });
    } 
    if (attendanceCount === 0) {
      res.status(200).send({ absent: 0, present: 0, students: studentCount,staffname: staffCount["Staff_name"],Class: staffCount["Assigned_Class"] });
    } else {
      res.status(200).send({ absent: absentPercentage, present: presentPercentage, students: studentCount,staffname: staffCount["Staff_name"],Class: staffCount["Assigned_Class"]});
    }
  } catch (error) {
    console.error('Error deleting record:', error);
    res.status(500).send({ message: 'Server error' });
...
const {subject} = req.body;
  try {
    ...
    if(sub){
  const stud = await collection2.find({Class:sub["Assigned_Class"],Section:sub["Section"]}).toArray()
  if(stud)
  {
...
app.post('/services/getsubjectsbystaffid', async (req, res) => {
  ...
  try {
    ...
    if (!staffMember) {
      return res.status(404).send({ message: 'No record found for the provided staff ID' });
    } else {
...
app.post('/services/getsubjectsbysid', async (req, res) => {
  ...
  try {
    ...
    const subjects = await staffCollection.find(
      { Assigned_Class: stud.Class }, 
      { projection: { Subject: 1 } }
    ).toArray();

    if (subjects.length === 0) {
      return res.status(200).send({ message: 'No subjects found for this class' });
    }
...
app.post('/services/getdataforpieadmin', async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({ absent: absentCount, present: presentCount, holidays: holidayCount });
  } catch (error) {
    console.error('Error getting Records:', error);
    res.status(500).send({ message: 'Server error' });
  } finally {
  ...
  try {  
    ...
    if (!searchusername) {
      ...
      return res.status(200).send({Message:"Student Not Found"});
        ...
        .then(result => {
          res.status(200).send({Message: "OTP Sent Successfully", OTP: result.otp,_id:searchusername["_id"],profile_pic:searchusername["isprofilepic"]}) 
      })
      .catch(error => {
  ...
  try {  
    ...
    if (!searchusername) {
      ...
      return res.status(200).send({Message:"Staff Not Found"});
        ...
        let otp =  genotp(searchusername["Staff_Email"],searchusername["Staff_name"]) 
        .then(result => {
          res.status(200).send({Message: "OTP Sent Successfully", OTP: result.otp,_id:searchusername["_id"],profile_pic:searchusername["isprofilepic"],subject:searchusername["Subject"]}) 
      })
      .catch(error => {
        res.status(200).send({Error: error}) 
        });
  ...
  try {  
    ...
    if (!searchadminemail) {
      ...
      return res.status(200).send({Message:"User Not Found"});
        ...
        let otp =  genotp(searchadminemail["E_mail"],searchadminemail["Staff_name"]) 
        .then(result => {
          res.status(200).json({
...
const updateadmindoc = async (stdid,picurl) =>{
  ...
  console.log(picurl)
    ...
    const updatepic  = await collection.updateOne({ _id: stdid}, 
    {
      $set: {
        isprofilepic: true, 
        profilepic: picurl
...
app.post("/services/addholidays", async (req, res) => {
  ...
  if (!Array.isArray(holidays) || holidays.length === 0) {
    return res.status(400).json({ message: "Invalid input: holidays array is required" });
  }
  ...
  try {
    ...
    for (const { name, date } of holidays) {
      if (!name || !date) {
        return res.status(400).json({ message: "Holiday name and date are required for each holiday" });
      }
      ...
      if (isNaN(parsedDate.getTime())) {
        return res.status(400).json({ message: `Invalid date for holiday: ${name}` });
      }
    ...
    if (holidayInsertions.length > 0) {
      ...
      res.status(201).json({ message: "Holidays added successfully", holidays: result.ops });
    } else {
      res.status(409).json({ message: "No new holidays to add. Some holidays already exist." });
    }
  } catch (error) {
...
app.post("/services/getholidays", async (req, res) => {
  ...
  try {
    ...
    const formattedHolidays = holidays.map(holiday => ({
      name: holiday.name,
      date: `${String(holiday.date.getDate()).padStart(2, '0')}/${String(holiday.date.getMonth() + 1).padStart(2, '0')}/${holiday.date.getFullYear()}`
    }));
