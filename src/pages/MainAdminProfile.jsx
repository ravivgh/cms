...
const MainAdminProfile = () => {
    ...
    const fetchProfileData = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5472/services/getadmindetails",
          {
            method: "POST",
  ...
  const onSubmit = async (data) => {
    try {
      const staffId = localStorage.getItem("admin_id");
      const response = await fetch(
        "http://localhost:5472/services/updateadmindetails",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          ...
          <div className="">
            <motion.div className="bg-gradient-to-r bg-[#f1ce8b] rounded-xl p-8 mb-8">
              <div className="flex items-center gap-6 flex-col">
                <motion.div>
                  <Avatar alt={profileData.Staff_name} src={`http://localhost:5472/profilepics/${localStorage.getItem("admin_id")}.png`} sx={{ width: 300, height: 300, borderRadius: "7px" }} className="rounded-2xl" />
                </motion.div>
                <div className="text-white">
                  ...
                  <p className="text-gray-600">{profileData.E_mail}</p>
              ...
              <CardContent className="space-y-4">
                <form onSubmit={handleSubmit(onSubmit)}>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label>Email</Label>
                      <Input {...register("E_mail", { required: "Email is required", pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: "Invalid email format" } })} readOnly={!isEditing} className={`${errors.Staff_Email ? "border-red-500" : ""} ${!isEditing ? "bg-gray-50" : "bg-white"}`} />
